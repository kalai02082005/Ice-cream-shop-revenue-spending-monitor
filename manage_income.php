...
if (!isset($_SESSION["email"])) {
    die("<p style='color:red'>You must be logged in to manage income records.</p>");
}
...
if (isset($_GET['delete_id'])) {
    ...
    $delete_sql = "DELETE FROM incomenew WHERE id = ?";
    $stmt = $conn->prepare($delete_sql);
    $stmt->bind_param("i", $delete_id);

    if ($stmt->execute()) {
        $message = "<div class='alert alert-success'>Income record deleted successfully!</div>";
    } else {
        $message = "<div class='alert alert-danger'>Error deleting income record. Please try again.</div>";
    }
...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $sql_income = "SELECT * FROM incomenew WHERE user_email = ? AND (category LIKE ? OR account LIKE ? OR description LIKE ?) ORDER BY date";
    $stmt = $conn->prepare($sql_income);
    $searchTerm = "%" . $search . "%";
    $stmt->bind_param("ssss", $email, $searchTerm, $searchTerm, $searchTerm);
} else {
    $sql_income = "SELECT * FROM incomenew WHERE user_email = ? ORDER BY date";
    $stmt = $conn->prepare($sql_income);
...
$row_income = $result_income->num_rows;
?>
