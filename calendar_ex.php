...
if (!isset($_SESSION['email'])) {
    die("You need to log in to view and add expense records.");
}
...
$query = "SELECT * FROM expensesnew WHERE MONTH(date) = '$month' AND YEAR(date) = '$year' AND user_email = '$_SESSION[email]'";
$result = mysqli_query($conn, $query);

$expenseRecords = [];
while ($row = mysqli_fetch_assoc($result)) {
    $day = date('j', strtotime($row['date']));
    $expenseRecords[$day][] = $row;
}
?>
...
<head>
    ...
    <title>Monthly Expense Calendar</title>
    <style>
       body{
...
<body>
    ...
    <table border="3">
        ...
        for ($day = 1; $day <= $daysInMonth; $day++) {
            ...
            if (isset($expenseRecords[$day])) {
                foreach ($expenseRecords[$day] as $record) {
